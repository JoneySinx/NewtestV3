...
# ─────────────────────────────────────────────────────────────────────────────
# 🎨 DASHBOARD SPECIFIC CSS (Search Bar & Dropdowns Only)
# ─────────────────────────────────────────────────────────────────────────────
CARD_CSS = """
<style>
/* ── Search zone ── */
...
.cdd-menu{position:absolute;top:calc(100% + 7px);left:50%;transform:translateX(-50%);min-width:max-content;background:var(--bg2,var(--bg3));border:1.5px solid var(--border);border-radius:16px;overflow:hidden;z-index:9999;box-shadow:0 8px 32px rgba(0,0,0,.45);animation:cddIn .15s ease;display:none}
@keyframes cddIn{
...
# ─────────────────────────────────────────────────────────────────────────────
# 🎬 DASHBOARD JS ENGINE — Synced with web_assets.py Global Logic
# ─────────────────────────────────────────────────────────────────────────────
JS_ENGINE = """
// Variables like curQ, curOff, pMode, etc. are already initialized globally in web_assets.py
...
async function doSearch(o){
    ...
    if(!q){showToastGlobal('Please enter a movie name','error');return;}
    ...
    try{
        ...
        if(!r.ok){showToastGlobal('Error fetching','error');return;}
        var d=await r.json();
        if(d.error){showToastGlobal(d.error,'error');return;}
        ...
        d.results.forEach(function(f){
            ...
            // Calls global `editFile` and `deleteFile` defined in web_assets.py
            var adminBtns='';
            if(d.is_admin){
        ...
        // Smart double pre-fetching
        if(nextOff) fetch('/api/search?q='+encodeURIComponent(q)+'&offset='+nextOff+'&col='+curCol+'&mode='+pMode);
    }catch(e){showToastGlobal('Network Sync Error','error');}
}
...
# ✅ OPTIMIZATION: Pre-compile the entire body string ONCE.
