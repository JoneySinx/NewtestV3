...
# ✅ MASTER IMPORTS: ग्लोबल रिस्पांस और UI अब web_assets से आयेंगे
from web.web_assets import build_page, get_auth, form_wrapper, fast_json_response

actor_routes = web.RouteTableDef()

# ─────────────────────────────────────────────────────────
# 🎨 MINI CSS & JS (सिर्फ डायरेक्टरी ग्रिड और गैलरी टैब्स के लिए)
# ─────────────────────────────────────────────────────────
PROFILE_EXTRAS = """
<style>
.dir-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:12px}@media(min-width:768px){.dir-grid{grid-template-columns:repeat(5,1fr);gap:20px}}
.act-card{background:var(--card);border:1px solid var(--border);border-radius:10px;overflow:hidden;transition:.2s;cursor:pointer}
.act-card:hover{transform:translateY(-6px);border-color:rgba(229,9,20,.6);box-shadow:0 8px 22px rgba(229,9,20,.25)}
.act-poster{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;transition:.4s}.act-card:hover .act-poster{transform:scale(1.1)}
...
.gallery-del-btn{position:absolute;bottom:8px;left:50%;transform:translateX(-50%);background:rgba(160,8,8,.85);border:1px solid var(--accent);color:#fff;padding:4px 10px;border-radius:4px;font-size:10px;cursor:pointer;opacity:0;transition:.15s}
.gallery-item-wrap:hover .gallery-del-btn{opacity:1}
.lightbox{position:fixed;inset:0;background:rgba(0,0,0,.92);backdrop-filter:blur(15px);z-index:99999;display:none;align-items:center;justify-content:center;opacity:0;transition:.2s}
.lightbox.open{display:flex;opacity:1}.lightbox-img{max-width:92%;max-height:88vh;object-fit:contain;border-radius:6px;transform:scale(.95);transition:.2s}
.lightbox.open .lightbox-img{transform:scale(1)}
</style>
"""

# ─────────────────────────────────────────────────────────
# 🌐 MAIN HOMEPAGE: ACTOR DIRECTORY
# ─────────────────────────────────────────────────────────
...
async def actors_directory_page(req):
    ...
    admin_btn = '''<button onclick="window.location.href='/admin/create_actor'" style="background:var(--accent); color:#fff; border:none; padding:10px 15px; border-radius:8px; font-weight:800; cursor:pointer; font-size:13px; margin-left:auto;">➕ Create Profile</button>''' if role == 'admin' else ""
    ...
    for a in all_actors:
        ...
        act_items += f'<div class="act-card" onclick="window.location.href=\'/actor/{str(a["_id"])}\'"><div style="position:relative; padding-top:135%; background:var(--bg3); overflow:hidden;"><img src="/api/actor/photo?id={str(a["_id"])}&v={v}" class="act-poster" loading="lazy"><div style="position:absolute; top:8px; left:8px; background:rgba(0,0,0,0.8); color:#fff; font-size:9px; padding:4px 8px; border-radius:4px; font-weight:800; z-index:2;">{i} {cat.capitalize()}</div></div><div style="padding:10px; text-align:center; font-size:13px; font-weight:800;">{html.escape(a.get("name", ""))}</div></div>'
    ...
    page_body = f'''
    {PROFILE_EXTRAS}
    <div class="main" style="padding-top:20px; max-width:1100px; margin:0 auto;">
        <div style="background:var(--card); border:1px solid var(--border); padding:16px; border-radius:12px; margin-bottom:25px; display:flex; gap:10px; flex-wrap:wrap;">
            ...
            <button onclick="searchDirectory()" style="background:var(--accent); color:#fff; border:none; padding:0 24px; border-radius:8px; font-weight:800; cursor:pointer;">Search</button>
            {admin_btn}
        </div>
        {initial_grid}
    ...
    <script>
    var dirOff=0, dirPage=1, hasNxt={'true' if has_next_init else 'false'};
    async function searchDirectory(){{
        var q=document.getElementById('dir_q').value, cat=document.getElementById('dir_cat').value;
        var r = await fetch(`/api/directory/search?q=${{encodeURIComponent(q)}}&cat=${{cat}}&offset=${{dirOff}}`);
        var d = await r.json(); document.getElementById('dir_grid_container').innerHTML = d.html; hasNxt = d.has_next;
        document.getElementById('dir_pg_box').style.display = (dirOff===0 && !hasNxt) ? 'none' : 'flex';
        document.getElementById('dir_pBtn').disabled=(dirOff===0); document.getElementById('dir_nBtn').disabled=!hasNxt;
        document.getElementById('dir_pgInfo').innerText = 'Page ' + dirPage;
    }}
    function nextDir(){{ if(hasNxt){{ dirOff+=20; dirPage++; searchDirectory(); window.scrollTo(0,0); }} }}
    function prevDir(){{ if(dirOff>0){{ dirOff=Math.max(0,dirOff-20); dirPage--; searchDirectory(); window.scrollTo(0,0); }} }}
    </script>'''
...
async def api_directory_search(req):
    role, _ = await get_auth(req)
    if not role: return fast_json_response({"html": ""})
    ...
    if not docs: return fast_json_response({"html": '<div style="color:var(--muted); text-align:center; padding:60px;">📇 No profiles found.</div>', "has_next": False})
    ...
    return fast_json_response({"html": html_out, "has_next": has_next})
...
async def actor_profile_display(req):
    role, _ = await get_auth(req)
    if not role: return web.HTTPFound('/login')
    try: actor = await actors.find_one({"_id": ObjectId(req.match_info['id'])})
    except: return web.Response(text="Invalid ID", status=400)
    if not actor: return web.Response(text="Profile Not Found", status=404)
        
    act_id, name, cat = str(actor["_id"]), actor["name"], actor.get("category", "actor")
    social, gallery = actor.get("social_links", {}), actor.get("gallery", [])
    
    t_html = "".join([f'<span style="background:var(--bg3); border:1px solid var(--border); color:var(--muted); font-size:11px; padding:3px 8px; border-radius:4px; font-weight:600; margin-right:5px;">#{html.escape(t)}</span>' for t in actor.get("tags", [])])
    ...
    admin_actions = f'''<div style="display:flex; gap:10px; margin-top:10px; flex-wrap:wrap;"><button onclick="document.getElementById('actorEditModal').classList.add('open')" class="pg-btn">✏️ Edit Details</button><button onclick="deleteProfile('{act_id}')" class="pg-btn" style="background:rgba(160,8,8,.78); color:#fff;">🗑️ Delete Profile</button><label class="pg-btn" style="cursor:pointer;">📸 Update Avatar<input type="file" onchange="updateAv('{act_id}', this)" style="display:none;"></label></div>''' if role == 'admin' else ""
    
    g_items = "".join([f'<div class="gallery-item-wrap" onclick="openLightbox(\'/api/actor/photo?id={act_id}&gallery_idx={i}\')"><img src="/api/actor/photo?id={act_id}&gallery_idx={i}" class="gallery-item">' + (f'<button class="gallery-del-btn" onclick="delGal(\'{act_id}\', {i}, event)">🗑️ Delete</button>' if role=='admin' else "") + '</div>' for i in range(len(gallery))])
    gallery_html = f'''<form action="/api/actor/gallery_upload" method="post" enctype="multipart/form-data" style="margin-bottom:20px; text-align:center;"><input type="hidden" name="actor_id" value="{act_id}"><label class="pg-btn" style="background:var(--accent); color:#fff;">📂 Upload to Gallery<input type="file" name="gallery_img" onchange="this.form.submit()" style="display:none;"></label></form>''' if role == 'admin' else ""
    ...
    # ✅ JAVASCRIPT: ग्लोबल `UniversalGridEngine` के बजाय छोटा कस्टमाइज्ड इंजन जो ग्लोबल CSS इस्तेमाल करेगा
    page_body = f'''
    {PROFILE_EXTRAS}
    <div class="main" style="padding-top:30px;max-width:1100px;margin:0 auto;">
        ...
        <div style="display:flex; gap:25px; background:var(--card); border:1px solid var(--border); padding:25px; border-radius:12px; margin-bottom:35px; flex-wrap:wrap;">
            <img id="masterAv" src="/api/actor/photo?id={act_id}&v={int(actor.get("photo_updated_at", 0))}" style="width:200px; aspect-ratio:3/4; border-radius:8px; object-fit:cover; border:1px solid var(--border);">
            <div style="flex:1; min-width:300px; display:flex; flex-direction:column; justify-content:center;">
                <h1 style="font-size:32px; font-weight:900; margin-bottom:5px;">{html.escape(name)}</h1>
        ...
        <div class="actor-tab-bar">
            <button class="actor-tab active" onclick="switchTab(event,'t-info')">ℹ️ Info</button>
            <button class="actor-tab" onclick="switchTab(event,'t-video')">🎬 Linked Media</button>
            <button class="actor-tab" onclick="switchTab(event,'t-gal')">🖼️ Gallery</button>
        </div>
        
        <div id="t-info" class="actor-panel active"><div style="background:var(--card); border:1px solid var(--border); padding:25px; border-radius:8px; white-space:pre-line;">{html.escape(actor.get("bio", ""))}</div></div>
        
        <div id="t-video" class="actor-panel">
            <div class="search-zone" style="padding:0; margin-bottom:15px;">
                <div class="search-row1" style="display:flex; gap:10px;">
                    <div class="search-wrap"><input class="search-input" id="act_q" placeholder="Search movies inside profile..."></div>
                    <button class="search-btn" onclick="actSearch(0)">Search</button>
                </div>
            </div>
            <div id="act_res" class="res-grid"></div>
            <div class="pagination" id="act_pg" style="display:none;"><button class="pg-btn" id="act_p" onclick="actPrev()">Prev</button><span class="pg-info" id="act_i">Page 1</span><button class="pg-btn" id="act_n" onclick="actNext()">Next</button></div>
        </div>
        
        <div id="t-gal" class="actor-panel">{gallery_html}</div>
    </div>
    
    <div id="lbx" class="lightbox" onclick="closeLightbox()"><button class="lightbox-close">&times;</button><img id="lbImg" class="lightbox-img" src=""></div>
    ...
    <div class="edit-modal" id="actorEditModal" onclick="if(event.target===this)this.classList.remove('open')">
        <div class="em-card"><button class="em-close" onclick="document.getElementById('actorEditModal').classList.remove('open')">&#10005;</button>
            ...
            <form action="/api/actor/update_profile" method="post"><input type="hidden" name="actor_id" value="{act_id}">
                <input type="text" name="name" value="{html.escape(name)}" class="em-input" required placeholder="Full Name">
                <textarea name="bio" class="em-input" style="min-height:100px" required placeholder="Bio">{html.escape(actor.get("bio", ""))}</textarea>
                <input type="text" name="tags" value="{html.escape(', '.join(actor.get("tags", [])))}" class="em-input" placeholder="Tags (Comma Separated)">
    ...
    <script>
    var aOff=0, aNext="", aPg=1;
    function switchTab(e, id){{ document.querySelectorAll('.actor-panel, .actor-tab').forEach(x=>x.classList.remove('active')); document.getElementById(id).classList.add('active'); e.currentTarget.classList.add('active'); if(id==='t-video' && !document.getElementById('act_res').innerHTML) actSearch(0); }}
    function openLightbox(s){{ document.getElementById('lbImg').src=s; document.getElementById('lbx').style.display='flex'; setTimeout(()=>document.getElementById('lbx').classList.add('open'),10); }}
    function closeLightbox(){{ document.getElementById('lbx').classList.remove('open'); setTimeout(()=>document.getElementById('lbx').style.display='none',200); }}
    
    // ✅ DRIVEN BY GLOBAL CSS & JS FROM web_assets.py
    async function actSearch(o){{
        var q=document.getElementById('act_q').value; aOff=o; if(o===0) aPg=1;
        var g = document.getElementById('act_res'); g.innerHTML='<div class="empty">Loading Matrix...</div>';
        try{{
            var r = await fetch(`/api/actor/search?q=${{encodeURIComponent(q)}}&offset=${{o}}&id={act_id}`); var d = await r.json();
            if(!d.results.length){{ g.innerHTML='<div class="empty">No linked media found.</div>'; document.getElementById('act_pg').style.display='none'; return; }}
            var h='';
            d.results.forEach(f => {{
                ...
                h += `<div class="file-card"><div class="poster-box" onclick="this.closest('.file-card').classList.toggle('admin-active');event.stopPropagation();"><img src="${{f.tg_thumb}}" class="fc-poster" onload="this.classList.add('loaded')"><div class="poster-top"><span class="type-chip">${{f.type}}</span><span class="size-chip">${{f.size}}</span></div>${{adm}}</div><div class="fc-body" onclick="window.open('${{f.watch}}','_blank')"><div class="fc-name">${{f.name}}</div></div></div>`;
            }});
            g.innerHTML=h; aNext=d.next_offset; document.getElementById('act_pg').style.display='flex';
            document.getElementById('act_p').disabled=(o===0); document.getElementById('act_n').disabled=!aNext; document.getElementById('act_i').innerText='Page '+aPg;
        }}catch(e){{}}
    }}
    function actNext(){{ if(aNext){{ aPg++; actSearch(aNext); }} }}
    function actPrev(){{ if(aPg>1){{ aPg--; actSearch(Math.max(0, aOff-20)); }} }}
    
    // Admin Actions
    async function updateAv(aid, inp){{ var f=inp.files[0]; if(!f) return; var fd=new FormData(); fd.append('actor_id',aid); fd.append('photo',f); var r=await fetch('/api/actor/update_avatar',{{method:'POST',body:fd}}); if((await r.json()).success) document.getElementById('masterAv').src='/api/actor/photo?id='+aid+'&v='+new Date().getTime(); }}
    async function delGal(aid, idx, e){{ e.stopPropagation(); if(confirm("Delete Photo?")){{ var r=await fetch('/api/actor/gallery_delete',{{method:'POST',body:JSON.stringify({{actor_id:aid,index:idx}}),headers:{{'Content-Type':'application/json'}}}}); if((await r.json()).success) window.location.reload(); }} }}
    async function deleteProfile(id){{ if(confirm("Delete Actor Profile?")){{ var r=await fetch('/api/actor/delete?id='+id,{{method:'POST'}}); if((await r.json()).success) window.location.href='/actors'; }} }}
    </script>'''
    return build_page(f"{name} - Directory Profile", page_body, "", "actors", role)

# ─────────────────────────────────────────────────────────
# ⚙️ LINKED MEDIA SEARCH API (FOR ACTOR PROFILE)
# ─────────────────────────────────────────────────────────
@actor_routes.get('/api/actor/search')
async def api_actor_search_handler(req):
    role, _ = await get_auth(req)
    if not role: return fast_json_response({"error": "Unauthorized"}, status=403)
    actor_id, q_custom, off = req.query.get("id"), req.query.get("q", "").strip(), int(req.query.get("offset", 0) if req.query.get("offset") else 0)
    
    if not actor_id: return fast_json_response({"results": []})
    actor = await actors.find_one({"_id": ObjectId(actor_id)})
    if not actor: return fast_json_response({"results": []})
    
    tags_list = actor.get("tags", [])
    search_query, final_tags = (q_custom, []) if q_custom else (tags_list[0] if tags_list else "", tags_list)
    if not search_query: return fast_json_response({"results": [], "next_offset": ""})
    ...
    # ✅ FIX: Added missing 'raw_collection' and 'caption' to enable Universal Editor
    results_list = [{
    ...
    return fast_json_response({"results": results_list, "next_offset": next_offset, "is_admin": role == "admin"})
...
async def api_create_actor(req):
    role, _ = await get_auth(req)
    if role != 'admin': return fast_json_response({"error": "Unauthorized"}, status=403)
    try:
...
async def api_actor_update_avatar(req):
    ...
    except: return fast_json_response({"success": False})
